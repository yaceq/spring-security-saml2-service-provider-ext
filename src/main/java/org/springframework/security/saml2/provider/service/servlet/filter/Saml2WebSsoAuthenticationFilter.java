...
public class Saml2WebSsoAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	...
	private final AuthenticationConverter artifactAuthenticationConverter;
	...
	public Saml2WebSsoAuthenticationFilter(RelyingPartyRegistrationRepository relyingPartyRegistrationRepository,
		...
		this(new Saml2AuthenticationTokenConverter(
				(RelyingPartyRegistrationResolver) new DefaultRelyingPartyRegistrationResolver(
						relyingPartyRegistrationRepository)), new Saml2ArtifactAuthenticationTokenConverter(
								new DefaultRelyingPartyRegistrationResolver(
										relyingPartyRegistrationRepository), null, null), 
				filterProcessesUrl);
	 ...
	 * @since 5.4
	 */
	public Saml2WebSsoAuthenticationFilter(AuthenticationConverter authenticationConverter, AuthenticationConverter artifactAuthenticationConverter, 
			String filterProcessesUrl) {
	...
	protected boolean requiresAuthentication(HttpServletRequest request, HttpServletResponse response) {
		return (super.requiresAuthentication(request, response)
				&& (StringUtils.hasText(request.getParameter(Saml2ParameterNames.SAML_RESPONSE)) 
						|| StringUtils.hasText(request.getParameter(Saml2ParameterNames.SAML_ART))));
	}
	...
	public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response)
		...
		if (StringUtils.hasText(request.getParameter(Saml2ParameterNames.SAML_ART))) {
			authentication = this.artifactAuthenticationConverter.convert(request);
		} else if (StringUtils.hasText(request.getParameter(Saml2ParameterNames.SAML_RESPONSE))) {
	 ...
	 * {@link Saml2ArtifactAuthenticationTokenConverter}, the
	...
	public void setAuthenticationRequestRepository(
		...
		setAuthenticationRequestRepositoryIntoAuthenticationConverters(authenticationRequestRepository);
	}

	private void setAuthenticationRequestRepositoryIntoAuthenticationConverters(
			Saml2AuthenticationRequestRepository<AbstractSaml2AuthenticationRequest> authenticationRequestRepository) {
		...
		if (this.artifactAuthenticationConverter instanceof Saml2ArtifactAuthenticationTokenConverter) {
			Saml2ArtifactAuthenticationTokenConverter authenticationTokenConverter = (Saml2ArtifactAuthenticationTokenConverter) this.artifactAuthenticationConverter;
			authenticationTokenConverter.setAuthenticationRequestRepository(authenticationRequestRepository);
		}
	}
