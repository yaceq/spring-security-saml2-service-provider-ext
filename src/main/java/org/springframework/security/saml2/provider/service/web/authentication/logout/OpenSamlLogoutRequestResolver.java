...
final class OpenSamlLogoutRequestResolver {
	...
	Saml2LogoutRequest resolve(HttpServletRequest request, Authentication authentication,
		...
		if (registration.getAssertingPartyDetails().getSingleLogoutServiceBinding() == Saml2MessageBinding.POST) {
		...
		else if (registration.getAssertingPartyDetails().getSingleLogoutServiceBinding() == Saml2MessageBinding.REDIRECT) {
			...
			OpenSamlSigningUtils.sign(logoutRequest, registration);
			return result.samlRequest(logoutRequest).relayState(relayState).build();
		}
	}
