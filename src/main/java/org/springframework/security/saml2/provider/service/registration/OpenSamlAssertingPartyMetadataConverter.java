...
class OpenSamlAssertingPartyMetadataConverter {
	...
	RelyingPartyRegistration.Builder convert(InputStream inputStream) {
		...
		RelyingPartyRegistration.Builder builder = RelyingPartyRegistration.withRegistrationId(descriptor.getEntityID())
				.validUntil(descriptor.getValidUntil())
		...
		for (SingleLogoutService singleLogoutService : idpssoDescriptor.getSingleLogoutServices()) {
			...
			else if (singleLogoutService.getBinding().equals(Saml2MessageBinding.SOAP.getUrn())) {
				binding = Saml2MessageBinding.SOAP;
			}
			else {
				continue;
			}
		...
		for (ArtifactResolutionService artifactResolutionService : idpssoDescriptor.getArtifactResolutionServices()) {
			Saml2MessageBinding binding;
			if (Boolean.FALSE.equals(artifactResolutionService.isDefault())) {
				continue;
			}
			if (artifactResolutionService.getBinding().equals(Saml2MessageBinding.SOAP.getUrn())) {
				binding = Saml2MessageBinding.SOAP;
			}
			else {
				continue;
			...
			builder.assertingPartyDetails(
					(party) -> party.artifactResolveServiceLocation(artifactResolutionService.getLocation())
							.artifactResolveServiceBinding(binding));
			break;
		}
		return builder;
	}
