...
import org.springframework.security.saml2.provider.service.web.DefaultSaml2ArtifactResolveContextResolver;
...
import org.springframework.security.saml2.provider.service.web.Saml2ArtifactResolveContextResolver;
...
public final class Saml2LoginConfigurer<B extends HttpSecurityBuilder<B>>
	...
	private AuthenticationConverter artifactAuthenticationConverter;
	 ...
	 * @since 5.4
	 */
	public Saml2LoginConfigurer<B> artifactAuthenticationConverter(AuthenticationConverter artifactAuthenticationConverter) {
		Assert.notNull(artifactAuthenticationConverter, "artifactAuthenticationConverter cannot be null");
		this.artifactAuthenticationConverter = artifactAuthenticationConverter;
		return this;
	}
	/**
	...
	public void init(B http) throws Exception {
		...
		this.saml2WebSsoAuthenticationFilter = new Saml2WebSsoAuthenticationFilter(getAuthenticationConverter(http),
				getArtifactAuthenticationConverter(http), this.loginProcessingUrl);
	...
	private AuthenticationConverter getArtifactAuthenticationConverter(B http) {
		if (this.artifactAuthenticationConverter != null) {
			return this.artifactAuthenticationConverter;
		}
		AuthenticationConverter authenticationConverterBean = getBeanOrNull(http,
				Saml2ArtifactAuthenticationTokenConverter.class);
		if (authenticationConverterBean == null) {
			Assert.state(this.loginProcessingUrl.contains("{registrationId}"),
					"loginProcessingUrl must contain {registrationId} path variable");
			RelyingPartyRegistrationResolver resolver = new DefaultRelyingPartyRegistrationResolver(
					this.relyingPartyRegistrationRepository);
			return new Saml2ArtifactAuthenticationTokenConverter(
					resolver,
					getArtifactResolveFactory(http),
					getArtifactResolveContextResolver(http));
		}
		return authenticationConverterBean;
	}
	
	private Saml2ArtifactResolveFactory getArtifactResolveFactory(B http) {
		Saml2ArtifactResolveFactory resolver = getSharedOrBean(http, Saml2ArtifactResolveFactory.class);
		if (resolver == null) {
	...
	private Saml2ArtifactResolveContextResolver getArtifactResolveContextResolver(B http) {
		Saml2ArtifactResolveContextResolver resolver = getBeanOrNull(http,
				Saml2ArtifactResolveContextResolver.class);
		if (resolver == null) {
