...
public final class TestOpenSamlObjects {
	...
	static ArtifactResponse artifactResponse() {
		return artifactResponse(DESTINATION, ASSERTING_PARTY_ENTITY_ID);
	}
	...
	static ArtifactResponse signedArtifactResponseWithOneAssertion() {
		return signedArtifactResponseWithOneAssertion((response) -> {
		});
	}

	static ArtifactResponse signedArtifactResponseWithOneAssertion(Consumer<ArtifactResponse> responseConsumer) {
		ArtifactResponse artifactResponse = artifactResponse();
		Response response = ((Response) artifactResponse.getMessage());
		response.getAssertions().add(assertion());
		responseConsumer.accept(artifactResponse);
		return signed(artifactResponse, TestSaml2X509Credentials.assertingPartySigningCredential(), RELYING_PARTY_ENTITY_ID);
	}
	...
	public static ArtifactResolve artifactResolve() {
		Issuer issuer = build(Issuer.DEFAULT_ELEMENT_NAME);
		issuer.setValue(RELYING_PARTY_ENTITY_ID);
		ArtifactResolve artifactResolve = build(ArtifactResolve.DEFAULT_ELEMENT_NAME);
		artifactResolve.setIssuer(issuer);
		artifactResolve.setDestination(ASSERTING_PARTY_ENTITY_ID + "/SSO.saml2");
	...
	public static LogoutResponse assertingPartyLogoutResponse(RelyingPartyRegistration registration) {
		...
		if (!registration.getAssertingPartyDetails().getSingleLogoutServiceBinding().equals(Saml2MessageBinding.SOAP)) {
